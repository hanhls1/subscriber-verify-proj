...
public enum MbfStatus {
	...
	MbfStatus(int value, String message) {
		this.value = value;
	...
	private static Map<Integer, MbfStatus> values = new HashMap<>();

	static {
		for (MbfStatus mbfStatus : values()) {
			values.put(mbfStatus.value, mbfStatus);
		}
	}

	public static MbfStatus valueOf(int value) {
		MbfStatus status = values.get(value);
